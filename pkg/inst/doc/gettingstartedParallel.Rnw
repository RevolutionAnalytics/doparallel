...
arguments, on Windows you will get a three-worker cluster and on Unix-like
...
using \texttt{doParallel} and \texttt{multicore}.  Note that you can register
...
Using \texttt{doParallel} and \texttt{multicore} we were able to perform
...
The \texttt{doParallel} package allows you to specify various options when
...
The \texttt{doParallel} and \texttt{multicore} packages provide a nice,
